...
//                        });
                    ...
                    if (options.addable) {
                        ...
                        if(!$('#globalBuId').val() && $('#isRoot').val() != 'Y') {
                            $(easyTree).find('.easy-tree-toolbar .operatebar .create > button').addClass("disabled");
                        }
                        ...
                        $(createInput).find('.confirm').click(function () {
                            ...
                            if (selected.length <= 0) {
                                ...
                                if(isMain == 'false') {
                                    ...
                                    var flag = true;
                                    if(options.dp) {
                                        if($(createInput).find('select').val() == '') {
                                            var newCategoryName = $(createInput).find('input').val();
                                            if(newCategoryName != '') {
                                                $(createInput).find('select option').each(function() {
                                                    if($(this).text() == newCategoryName) {
                                                        var adjustBlock = $(easyTree).find('.actionbar');
                                                        $(adjustBlock).html(warningAlert);
                                                        $(adjustBlock).find('.alert .alert-content').text(options.i18n.newCategoryExist);
                                                        flag = false;
                                                    }
                                                });
                                            }
                ...
                $(this.easyTree).each(function () {
                    ...
                    if (selected.length <= 0) {
                        ...
                        if(adjustType == ADJUST_TYPE) {
                            ...
                            $(targetNode).find('#' + selectedId).find('li').addClass('disabled');
